...
def analytic(T_i, T_inf, t, a):
    """This can be generated with Python symbolic math package, sympy.
    """
...
def implicit(T_i, T_inf, dt, L, A, m_obj, cp_obj):
    ...
    def to_zero(T_end_guess):
        ...
        a = h*A/(m_obj*cp_obj)
        b = eps*sigma*A/(m_obj*cp_obj)
        T_new = (T_i - a*(T_end_guess -T_inf)*dt 
                     - b*(T_end_guess**4 -T_inf**4)*dt )
        return T_new - T_end_guess
...
# print("Write for loop here and call function explicit")

for i in range(len(t)-1):
    T_explicit[i+1] = explicit(T_explicit[i], T_inf, dt, L_vertical, 
                               A, m_obj, cp_obj)
    T_implicit[i+1] = implicit(T_implicit[i], T_inf, dt, L_vertical, 
                               A, m_obj, cp_obj)
