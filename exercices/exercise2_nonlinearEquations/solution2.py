...
def haaland(Re, eps_over_d):
    ...
    """
    f  = (-1.8*sp.log10((eps_over_d/3.7)**1.11+6.9/Re))**-2
    return f
...
def heat_transfer_coefficient_vertical_infinite_cavity(T, p, L, fluid):
    ...
    Ra = (beta*g*L**3*(sp.absolute(T[0]-T[1]) )) / (nu**2) * Pr
...
def energy_balance(T):
    """Returns energy inbalance for plates 1 and 2. Make this zero.
...
#    h_i0 = heat_transfer_coefficient_natural_convection_vertical_plate(
#            T[0], T_inf,L, p, fluid)
...
T_initial = sp.array([260, 160])+C2K
print("Energy inbalance", energy_balance(T_initial))

########################################
# (b) Use root
########################################
#T_initial = sp.array([300, 200])+C2K
