...
def energy_balance(T):
    ...
    f = np.zeros(2)
    f[0] = (
               A*eps*sigma*T[0]**4             # Object radiates out
             - A*eps*sigma*T[1]**4*F       # The other object radiates in
             - A*eps*sigma*T_inf**4*(1-F)  # Enviroment radiates in
             + A*h1*(T[0] - T_inf)             # Convective heat transfer out
             - Q1                              # Electric heating in
           )
    f[1] = (
              A*eps*sigma*T[1]**4 
            - A*eps*sigma*T[0]**4*F 
            - A*eps*sigma*T_inf**4*(1-F)
            + A*h2*(T[1] - T_inf)
            - Q2
           )
    return f
