...
def read(path, first_index=0, last_index=1e99999):
    ...
    return np.array(t)-t[0], np.array(T_red)+C2K, np.array(T_sensor)+C2K
...
def ode(t,T,T_inf, L, A, m_obj, cp_obj):

    # With radiation and calculated h
    h = hVertical(T, T_inf, L)
    a = h*A/(m_obj*cp_obj)
    b = eps*sigma*A/(m_obj*cp_obj)
    return( -a*(T - T_inf) - b*(T**4 - T_inf**4) )
...
#--------------Runge-Kutta45----------------------------------------    
# max_step=dt required for stability. hVertical crashes other ways
# With BDF, for example, max_step is not needed and less nfev is required
sol = solve_ivp(lambda t,T: ode(t,T,T_inf, L_vertical, A, m_obj, cp_obj), 
                [0,t_end], [T_0],
                method='RK45', t_eval=t, max_step=dt)
#print(sol)
assert sol.success


# Non-lambda way of using solve_ivp. lambda creates a nameless function.
#def temp(t, T):
#    return ode(t,T,T_inf, L_vertical, A, m_obj, cp_obj)
#sol = solve_ivp(temp, 
#                [0,t_end], [T_0],
#                method='RK45', t_eval=t, max_step=dt)

# solve_ivp is a general type solver for n-dimensons, i.e. returns arrays
T_rk45 = sol.y[0] 
...
plt.plot(t/60, T_rk45-C2K, 'bo', markersize=4, label='RK45')
